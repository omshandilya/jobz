...
class NaukriScraper(BaseScraper):
    def search(self, query: str, location: str, date_hours: int = 24) -> List[Dict[str, Any]]:
        ...
        with sync_playwright() as p:
            # Use Chrome channel if available, fallback to Chromium, then Firefox
            browser = None
            try:
            ...
            if _STEALTH_AVAILABLE:
                stealth_sync(page)
                logger.info("NaukriScraper: playwright-stealth applied.")
            ...
            try:
                ...
                for page_num in range(1, 3):
                    ...
                    job_elements = page.query_selector_all(
                        ...
                        "[class*='srp-jobtuple'], [class*='cust-job-tuple']"
                    )
                    if not job_elements:
            ...
            for raw_job in raw_jobs[:10]:
                try:
                    ...
                    # Extract company domain — iterate all links and find a non-Naukri external URL
                    company_domain = ""
                    try:
                        ...
                        for link in all_links:
                            try:
                                ...
                                # Must be an external HTTP link not pointing back to naukri.com
                                if (
                                    href.startswith("http")
                                    and "naukri.com" not in href
