...
def filter_jobs(jobs: list) -> list:
    ...
    On API error or unconfigured key: sets score=0.5, skills=[], and EXCLUDES the job.
    """
...
Return only JSON: {{"score": 0.85, "skills": ["Python", "FastAPI"]}}"""
        ...
        if client:
            try:
                ...
                # On API error: set score=0.5, skills=[], exclude the job
                job["relevancy_score"] = 0.5
                job["skills_extracted"] = []
        else:
            # No Groq client configured: set score=0.5, skills=[], exclude the job
            job["relevancy_score"] = 0.5
            job["skills_extracted"] = []

        # Only include jobs that genuinely scored >= 0.6
